...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Sanitize and assign form values for patient information
    ...
    // FIX: Properly handle the 'when' date field
    if (!empty($_POST['when']) && $_POST['when'] !== '0000-00-00') {
        // Convert the input date to MySQL format (YYYY-MM-DD)
        $when = date('Y-m-d', strtotime($_POST['when']));
        
        // Additional validation to ensure the date is valid
        if ($when === false || $when === '1970-01-01') {
            // Invalid date format or conversion failed
            $when = null;
        }
    } else {
        $when = null;
    ...
    // For debugging - comment out in production
    // echo "Date value being inserted: " . ($when ? $when : "NULL") . "<br>";
    ...
    // Bind the parameters
    $stmt->bind_param("isssisssssssssssssississiiiissiississ", 
